...
Servo shoulder; // 360-degree Servo
Servo elbow; // 360-degree Servo
Servo base; // 180-degree MicroServo
Servo gripper; //180-degree MicroServo
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (x >= 0 && x <= 360) { // Adjusted for 360-degree servo
      shoulderTo(x);
    }

    if (x >= 400 && x <= 760) { // Adjusted for 360-degree servo
      Serial.println("done");
      while (Serial.available() == 0) { }
      float y = Serial.readString().toFloat();
      if (y >= 0 && y <= 360) { // Adjusted for 360-degree servo
        comboMove(y, x - 400); // Adjusted offset
      }
    }
...
void comboMove(float starget, float etarget) {
  int sdir = (starget >= shoulder.read()) ? 1 : -1;
  int edir = (etarget >= elbow.read()) ? 1 : -1;

  while (abs(starget - shoulder.read()) > 1 || abs(etarget - elbow.read()) > 1) {
    if (abs(etarget - elbow.read()) > 1) {
      elbow.write(elbow.read() + edir);
      delay(20);
    }
    if (abs(starget - shoulder.read()) > 1) {
      shoulder.write(shoulder.read() + sdir);
      delay(20);
    }
  }
}
...
void rest() {
  shoulderTo(180); // Adjusted for 360-degree servo
  elbowTo(90); // Adjusted for 360-degree servo
}
