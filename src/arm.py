...
# class ChessRobotArm:
#     def __init__(self, a: int, b: int, port='COM5', verbose=False) -> None:
#         self.port = port
...
#         if len(square) != 2 or square[0] not in filemap or not square[1].isdigit():
#             print(f"Invalid square input: '{square}'")
#             return
#         x = filemap[square[0]]
#         y = int(square[1]) + 0.5
...
#     robot = ChessRobotArm(22, 22.5, port='COM5', verbose=True)

#     while True:
#         sq = input("Enter a chess square (e.g., 'e2'): ").strip().lower()
#         if not sq:
#             print("Empty input received, please enter a valid square.")
#             continue
#         robot.drop()
#         robot.go_to(sq)
#         robot.grasp()
...
class ChessRobotArm:
    ...
    def elbow_to(self, angle: int) -> None:
        self.command(angle + 400, wait=False)  # Adjusted offset
    
    def combo_move(self, sangle, eangle):
        self.command(eangle + 400)  # Adjusted offset
        self.command(sangle)
    
    def base_to(self, angle: int) -> None:
        self.command(angle + 800)  # Adjusted offset
    ...
    def go_to(self, square: str):
        ...
        # Square-specific Tuning
        # Add any specific adjustments for the 360-degree servos here
        # (These adjustments depend on testing and should be done as needed)
